...
| fiservBillpaySubscriberId | Req | body | string | Used by the user to authenticate with the user’s product(s). <br> Recommended Length: 9–25 <br> Although 2–32 characters are technically allowed, Fiserv strongly recommends 9–25. If a Subscriber ID exceeds 25 characters, restrictions may cause the Subscriber ID to be truncated. <br> Pattern: ^\[a-zA-Z0-9_(){}&amp;@!+#.'$,%^ \*-\]\* <br> Best practices: This ID should contain at least nine characters. Do not include special characters (such as # or &amp;). Do not use a Social Security number. |
 | fiservBillpayExternalSubscriberId | Req | body | string | A unique customer ID, different from the fiservBillpaySubscriberId, that is used for logging the user in to a Fiserv product. Length: 2–48 <br>  Pattern: ^\[a-zA-Z0-9_(){}&amp;@!+#.'$,%^ \*-\]\* <br>  Condition: Required if any of the consumer’s products require it. <br> Best practices: This ID should contain at least nine characters. Do not include special characters (such as # and &amp;). Do not use a Social Security number. |
